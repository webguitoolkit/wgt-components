...
/**
 * replaces select-control for large selections. load this view into an abstratcDialog
 * 
 * @author BK
 * 
 */
public class SelectionHelper extends AbstractView {
	...
	private class SelectionListener extends AbstractTableListener implements IActionListener {
			...
			if (event.getSource() == cancel) {
				...
				if (valueTable != null) {
					...
					if (selected.isEmpty()) {
						...
						ServerEvent serverevent = new ServerEvent(SelectionHelper.SELECTION_EVENT);
						serverevent.putParameter(SelectionHelper.SELECTION_EVENT_PARAMETER, selected);
		...
		public void onRowSelection(ITable table, int row) {
			...
			if (model != null && model.getSelectionMode() == SelectionHelperModel.SELECTION_MODE_SINGLE) {
				if (((Table)table).getSelectedRowIndex() > -1) {
					...
					ServerEvent serverevent = new ServerEvent(SelectionHelper.SELECTION_EVENT);
					serverevent.putParameter(SelectionHelper.SELECTION_EVENT_PARAMETER, bag);
					fireServerEvent(serverevent);
					getViewConnector().getWindowActionListener().onClose(event);
				}
				else {
		...
		public void onEditTableLayout(ClientEvent event, int rowCount, String tableSetting) {
			...
			ServerEvent serverevent = new ServerEvent(SelectionHelper.TABLE_LAYOUT_EDIT_EVENT);
			serverevent.putParameter(SelectionHelper.TABLE_LAYOUT_EDIT_EVENT_PARAMETER, tableSetting);
			fireServerEvent(serverevent);			
		}
	}
	...
	private class SearchListener implements IActionListener {
		public void onAction(ClientEvent event) {
			if (event.getSource() == search) {
				...
				ServerEvent serverevent = new ServerEvent(SelectionHelper.SEARCH_EVENT);
				...
				searchbag.save();
				searchbag.addProperty(SEARCH_EVENT_PARAMETER_HELPER, SelectionHelper.this);
				serverevent.putParameter(SelectionHelper.SEARCH_EVENT_PARAMETER_BAG, searchbag);
				fireServerEvent(serverevent);
			}
		}
	}

	public SelectionHelper(WebGuiFactory factory, ICanvas viewConnector, SelectionHelperModel model) {
	...
	protected void createControls(WebGuiFactory factory, ICanvas viewConnector) {
		...
		if (model != null && model.isSearchEnabled() && model.isValid()) {
			searchCompound = getFactory().newCompound(grid);
			{
				...
				for (int i = 0; i < model.getColumns().length; i++) {
					...
					if (model.isSearchable(model.getColumns()[i])) {
						if (model.getReourceKey(model.getColumns()[i]) != null) {
							...
							l = getFactory().newLabel(searchGrid, SelectionHelper.createName(model.getColumns()[i]));
						}
				...
				// add possible additional searchfields
					...
					while (iAdd.hasNext()) {
						...
						if (model.isSearchable(key) && !model.isInColumns(key)) {
							...
							if (model.getReourceKey(key) != null) {
								...
								l = getFactory().newLabel(searchGrid, SelectionHelper.createName(key));
							}
							if (model.getSelectValues(key) != null) {
								// render select
								Select input = getFactory().newSelect(searchGrid, key);
								input.setDescribingLabel(l);
								input.loadList(model.getSelectValues(key));
							}
							else {
								// render text
		...
		if (model != null && model.isValid()) {
			...
			for (int i = 0; i < model.getColumns().length; i++) {
				if (model.getReourceKey(model.getColumns()[i]) != null) {
					getFactory().newTableColumn(valueTable, TextService.getString(model.getReourceKey(model.getColumns()[i])), model.getColumns()[i],
							!model.isSearchEnabled());
				}
				else {
					getFactory().newTableColumn(valueTable, SelectionHelper.createName(model.getColumns()[i]), model.getColumns()[i],
							!model.isSearchEnabled());
				}
			}
